...
class NeighborhoodAdaptiveContext(nn.Module):
    ...
    包含 '零初始化' (Zero Initialization) 策略，防止破坏预训练特征。
    """
    def __init__(self, embed_dim, num_heads=8, kernel_size=7, scale=1.0):
        ...
        self.adapt_mlp = nn.Sequential(
            ...
            nn.Linear(embed_dim, 2), # Index -2: 最后一层线性层
            nn.Softmax(dim=-1)
        )
        ...
        self.ffn = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(embed_dim * 4, embed_dim), # Index -1: 最后一层
        )
    ...
    def _init_weights(self):
        """
        零初始化策略 (Zero/Bias Initialization)
        目的：确保模块在初始状态下等价于恒等映射 (Identity Mapping)，
        完全保留原始特征，避免破坏预训练的 Backbone 分布。
        """
        # 1. 偏置门控网络 (Bias the Gating)
        # 目标：使初始权重 weights ≈ [0, 1] (Context=0, Original=1)
        # 获取 adapt_mlp 的最后一层线性层 (在 Softmax 之前)
        last_linear = self.adapt_mlp[-2] 
        
        # 将权重设为 0，消除输入的随机影响
        nn.init.constant_(last_linear.weight, 0)
        
        # 设置偏置：
        # bias[0] (Context) = -5.0  -> exp(-5) ≈ 0.0067
        # bias[1] (Original) = 5.0  -> exp(5)  ≈ 148.4
        # Softmax 后 -> [0.00004, 0.99996]
        with torch.no_grad():
            last_linear.bias[0] = -5.0
            last_linear.bias[1] = 5.0
        
        # 2. 零初始化输出投影 (Zero Init Projection)
        # 确保 aggregated context 最初为 0
        nn.init.constant_(self.out_proj.weight, 0)
        nn.init.constant_(self.out_proj.bias, 0)

        # 3. 零初始化 FFN (Zero Init FFN)
        # 确保 FFN 分支最初贡献为 0
        nn.init.constant_(self.ffn[-1].weight, 0)
        nn.init.constant_(self.ffn[-1].bias, 0)
        
        # (可选) 这里的 context_proj 可以保留默认初始化，让它去学特征提取

    def forward(self, lidar_feat):
        B, C, H, W = lidar_feat.shape
        
        # --- 1. Context Construction ---
        ctx_feat = self.context_proj(lidar_feat)
        ...
        # 经过 Zero Init 的 FFN，此时 fused_ffn ≈ 0
