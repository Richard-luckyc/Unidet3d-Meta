...
# ---------------- NeighborhoodCrossAttentionFusion (结构完整保留) ----------------
# 为了保证 checkpoint 能够加载，不要修改类名和内部层定义
class NeighborhoodCrossAttentionFusion(nn.Module):
    ...
    def forward(self, lidar_feat, image_feat):
        # 注意：实际上 extract_feat 中将不会再调用此函数
        # 但保留此逻辑以防万一
...
class UniDet3D_Meta(MetaModule, Base3DDetector):
    def __init__(self,
        ...
        # image backbone（保留初始化，确保 checkpoint keys 匹配）
        ...
        # cross-modal fusion (保留初始化，确保 checkpoint keys 匹配)
    ...
    def extract_feat(self, x, superpoints, inverse_mapping, batch_offsets, image_input=None):
        """
        修改后的 extract_feat：
          - 即使 image_input 存在，也强制忽略
          - 仅执行 ModuleA 增强的纯 LiDAR 流程
          - 确保 Checkpoint 正常加载，但图像特征不参与计算
        """
        ...
        # =========================================================================
        # [修改点] 彻底删除图像融合逻辑块
        # 即使 hasattr(self, 'image_backbone') 为 True (为了加载权重)，我们也不调用它。
        # 这确保了行为等同于 ModuleA-only 模式，同时不破坏权重加载。
        # =========================================================================
    ...
    def trim_bboxes_by_superpoints(self, sp_pts_mask, point, 
        ...
        # [关键修复] 处理零检测情况，防止 torch_scatter 报错
        if n_boxes == 0:
