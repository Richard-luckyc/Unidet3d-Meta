...
class NeighborhoodCrossAttentionFusion(nn.Module):
    def __init__(self, embed_dim, num_heads=8, kernel_size=7, dropout=0.1):
        ...
        # [修改 1] 输入通道从 2048 改为 embed_dim
        # 这样它就可以接受 LiDAR 特征作为输入，实现自注意力
        self.img_proj = nn.Conv2d(embed_dim, embed_dim, kernel_size=1)
    ...
    # [修改 2] 移除 image_feat 参数，仅接受 lidar_feat
    def forward(self, lidar_feat):
        B, C, H, W = lidar_feat.shape
        
        # [逻辑变更] 使用 LiDAR 特征自身作为上下文源 (Self-Attention)
        # 投影: [B, C, H, W] -> [B, C, H, W]
        img_feat_proj = self.img_proj(lidar_feat)  
        
        pad = self.kernel_size // 2
        unfold = nn.Unfold(kernel_size=self.kernel_size, padding=pad)
        
        # 提取投影后的 LiDAR 特征邻域
        img_unfold = unfold(img_feat_proj)  # [B, C * K*K, L]
        lidar_flat = lidar_feat.view(B, C, -1).permute(0, 2, 1)  # [B, L, C]
        
        K2 = self.kernel_size * self.kernel_size
        img_unfold = img_unfold.view(B, C, K2, H*W).permute(0, 3, 2, 1)  # [B, L, K*K, C]
        ...
        fused = (attn_weights * img_unfold).sum(dim=2)
        fused = fused.permute(0, 2, 1).view(B, C, H, W)
        fused = self.out_proj(fused)
        
        # 自适应门控: 比较原始 LiDAR 和 聚合后的 LiDAR
        feat_concat = torch.cat([lidar_feat.mean([2,3]), img_feat_proj.mean([2,3])], dim=1)
...
class UniDet3D_Meta(MetaModule, Base3DDetector):
    def __init__(self,
        ...
        # Image Backbone 初始化 (可选: 如果不再使用可以注释掉，或者保留以兼容旧权重)
    ...
    def extract_feat(self, x, superpoints, inverse_mapping, batch_offsets, image_input=None):
        ...
        for sample_idx, lf in enumerate(lidar_feats):
            ...
            try:
                ...
                # 3.3: NAC 模块增强 (Active, Self-Attention)
                # [关键] 移除 image_feat 参数，直接调用
                lf_final_2d = self.cross_modal_fusion(lf_enh)

                # 3.4: 还原到 1D
                lf_final_1d = lf_final_2d.view(C, -1).transpose(0, 1).contiguous()
                lf_final_1d = lf_final_1d[:N_i, :]
                processed_feats.append(lf_final_1d)

            except Exception as e:
                print(f"[FeatExtract WARNING] Sample {sample_idx} failed: {e}")
                processed_feats.append(lf)
    ...
    def trim_bboxes_by_superpoints(self, sp_pts_mask, point, 
        ...
        # [关键] 防御性编程：处理零检测结果
        if n_boxes == 0:
             return [(DepthInstance3DBoxes(bboxes.new_zeros(0, 6), with_yaw=False), labels, scores)]
