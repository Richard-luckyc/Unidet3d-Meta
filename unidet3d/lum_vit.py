...
class LUM_ViT(VisionTransformer):
    def __init__(self, target_rate=0.1, **kwargs):
        kwargs.pop('num_patches', None) # 移除不被 VisionTransformer 接受的参数
    ...
    # 如果输入是 spconv 的 SparseTensor，则转换为密集张量
        if hasattr(x, 'dense'):
            x = x.dense()

    # 检查输入维度：如果是 5D (B, C, D, H, W)，则取中间深度切片转换为 4D
        if x.dim() == 5:
            B, C, D, H, W = x.shape
            x = x[:, :, D // 2, :, :]
        elif x.dim() > 5:
        # 若有多余维度，可尝试 squeeze 掉冗余维度（需确保该维度大小为1）
            x = x.squeeze(2)
        # 在进入 patch_embed 前调整尺寸为模型期望的 img_size
        expected_size = self.patch_embed.img_size  # 例如 (208, 208)
        x = F.interpolate(x, size=expected_size, mode='bilinear', align_corners=False)
    # 如果转换后仍非 4D，则报错（便于调试）
        if x.dim() != 4:
            raise ValueError(f"Expected 4D tensor for patch_embed, but got shape: {x.shape}")
